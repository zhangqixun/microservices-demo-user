...
func (s *fixedService) Login(username, password string) (users.User, error) {
	...
	fmt.Println("enter service.Login")
	// Log domain.
	var logger log.Logger
	{
		logger = log.NewLogfmtLogger(os.Stderr)
		logger = log.With(logger, "[APPINFO]ts", log.DefaultTimestampUTC)
		logger = log.With(logger, "caller", log.DefaultCaller)
	}

	logger.Log("msg", "enter service.Login")

	u, err := db.GetUserByName(username)
	logger.Log("msg", "db.GetUserByName")
	if err != nil {
		return users.New(), err
	}
	if u.Password != calculatePassHash(password, u.Salt) {
		logger.Log("username", username, "Unauthorized", "1")
		return users.New(), ErrUnauthorized
	}

	db.GetUserAttributes(&u)
	logger.Log("msg", "GetUserAttributes success")
	u.MaskCCs()
	logger.Log("msg", "MaskCCs success")
	return u, nil
...
func (s *fixedService) GetUsers(id string) ([]users.User, error) {
	...
	logger.Log("msg", "enter service.GetUsers")
